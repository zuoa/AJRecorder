...
class Ass:
    def __init__(self, danmu_list, video_start_time, ass_file_path):
        self.danmu_list = danmu_list
        self.video_start_time = video_start_time
        if not os.path.exists(os.path.dirname(ass_file_path)):
            os.makedirs(os.path.dirname(ass_file_path))
        self.fp = open(ass_file_path, "w")
...
Style: R2L,PingFang SC,14,&H33FFFFFF,&H99FFFFFF,&H99000000,&H99000000,1,0,0,0,100,100,0,0,1,1,0,2,20,20,2,0
...
"""
    ...
    def flush(self):
        ...
        tracks = [None for _ in range(int((DANMU_VIEWPORT_HEIGHT - DANMU_VIEWPORT_OFFSET_Y * 2) / LINE_HEIGHT))]
        for index, dm in enumerate(self.danmu_list):
            ...
            while i < len(tracks):
                if tracks[i] is None or danmu.compare(tracks[i]):
                    tracks[i] = danmu
                    danmu.y = (i + 1) * LINE_HEIGHT + DANMU_VIEWPORT_OFFSET_Y
                ...
                print(f"丢弃第{i + 1}条弹幕。{danmu.content}")
...
class AssDanmu:
    def __init__(self, msg_time, content, color, video_start_time):
        ...
        self.end_x = -FONT_SIZE * len(content)
    ...
    def compare(self, danmu_before):
        ...
        distance = (self.start_time - danmu_before.start_time) * danmu_before.velocity - (
                len(self.content) + len(danmu_before.content)) / 2 * FONT_SIZE
        ...
        max_pursue_time = 12000 - (self.start_time - danmu_before.start_time)
